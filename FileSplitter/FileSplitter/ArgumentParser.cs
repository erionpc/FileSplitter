    ...
    public class ArgumentParser : IArgumentParser
    {
        ...
        public OperationOptionsEnum GetOperation()
        {
            ...
            if (!recognisedOperationEnums.Select(x => x.GetAttribute<ArgumentInfo>().ArgumentSwitch).Contains(Arguments[0].ToLower()))
                throw new FileSplitException($"Unrecognised operation. The first argument needs to be one of the following:{Environment.NewLine}" +
        ...
        public FileSplitInfo BuildFileSplitInfo()
        {
            ...
            if (recognisedSwitchesInArgs.Count != switchesInArgs.Count)
                throw new FileSplitException($"Unrecognised split option: {string.Join(", ", switchesInArgs.Except(recognisedSwitchesInArgs))}");

            if ((switchesInArgs?.Count ?? 0) > (switchesInArgs?.Select(x => x?.ToLower())?.Distinct()?.Count() ?? 0))
                throw new FileSplitException($"Duplicated options");
        ...
        public FileMergeInfo BuildFileMergeInfo()
        {
            ...
            if (recognisedSwitchesInArgs.Count != switchesInArgs.Count)
                throw new FileSplitException($"Unrecognised merge option: {string.Join(", ", switchesInArgs.Except(recognisedSwitchesInArgs))}");
            ...
            if (string.IsNullOrWhiteSpace(filePath))
                throw new FileSplitException($"{destinationFilePathArgument.ArgumentDescription} not specified");

            string filePartsString = GetArgument(filePartsArgument.ArgumentSwitch);
            if (string.IsNullOrWhiteSpace(filePartsString))
                throw new FileSplitException($"{filePartsArgument.ArgumentDescription} not specified");

            var fileParts = filePartsString.Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).AsEnumerable();
